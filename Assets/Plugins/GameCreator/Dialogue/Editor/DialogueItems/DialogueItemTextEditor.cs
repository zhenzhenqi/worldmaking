namespace GameCreator.Dialogue
{
    ...
    public class DialogueItemTextEditor : IDialogueItemEditor
    {
        ...
        public static void AddElement(DialogueEditor dialogueEditor)
        {
            ...
            else if (dialogueEditor.editorRoot.spChildren.arraySize > 0)
            {
                Debug.Log("SIZE OF ROOT CHILDREN: " + dialogueEditor.editorRoot.spChildren.arraySize);
                int index = dialogueEditor.editorRoot.spChildren.arraySize;
                SerializedProperty lastRoot = dialogueEditor.editorRoot.spChildren.GetArrayElementAtIndex(index - 1);

                if (lastRoot.objectReferenceValue == null)
                {
                ...
                {
                    lastRoot.objectReferenceValue.GetInstanceID()
                };
            }
